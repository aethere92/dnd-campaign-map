<h3>I. Session Overview</h3>
<ul>
	<li>
		<strong>Session Number:</strong>
		1
	</li>
	<li>
		<strong>In-Game Date/Time:</strong>
		Day 4 of travel, afternoon.
	</li>
	<li>
		<strong>Location(s) Explored:</strong>
		Road to Drellin's Ferry, Abandoned Farmhouse, Drellin's Ferry (Tollhouse, Shrine of Pelor, Sertieren's Mansion)
	</li>
	<li>
		<strong>The Big Headline:</strong>
		The party survived a hobgoblin ambush and arrived at Drellin's Ferry, where they learned of the Wyrmlord Koth and
		his "Day of Ruin" plan while beginning investigations into local threats.
	</li>
</ul>
<h3>II. Narrative Summary</h3>
<p><strong>Travel and Ambush</strong></p>
<ul>
	<li>Party traveling toward Drellin&rsquo;s Ferry through subtropical forest.</li>
	<li>Encountered abandoned farmhouse in shallow dell; Bonnie identifies it as potential ambush site.</li>
	<li>Ambushed by a group of hobgoblins:</li>
	<ul>
		<li>Terrain and fatigue make battle challenging.</li>
		<li>
			Party coordinates attacks: Norr (lightning), Soshi (fire), Kaedin (echo strike), Olek (melee), Bonnie (dagger).
		</li>
		<li>Enemy captain killed; other enemies retreat.</li>
	</ul>
	<li>Captured one hobgoblin survivor:</li>
	<ul>
		<li>Interrogation initially fails (hobgoblin silent).</li>
		<li>Olek casts Zone of Truth; hobgoblin finally agrees to answer in exchange for promised swift death.</li>
		<li>Answers:</li>
		<ul>
			<li>Warlord: Wyrmlord Koth, a captain gathering tribes for &ldquo;Day of Ruin.&rdquo;</li>
			<li>Plans: vague, details unclear.</li>
			<li>Location: unknown/refused.</li>
		</ul>
		<li>Hobgoblin released and killed by Olek per promise.</li>
	</ul>
	<li>Party burns three dead travelers&rsquo; bodies to prevent misuse, then carefully extinguishes fire.</li>
</ul>
<p><strong>Encounter with Civilians</strong></p>
<ul>
	<li>Farmer and two sons walk toward the farmhouse:</li>
	<ul>
		<li>Olek tries to warn them; they panic at his appearance.</li>
		<li>Bonnie telepathically frightens them (&ldquo;Boo&rdquo;), causing them to flee.</li>
		<li>Bonnie collects supplies left behind; Olek returns most items except spoiled wine.</li>
	</ul>
</ul>
<p><strong>Arrival at Drellin&rsquo;s Ferry</strong></p>
<ul>
	<li>Town description: along river, ferryboats replace missing bridge, orchards, fields, five watchtowers.</li>
	<li>Met town guards led by Sergeant Hersk:</li>
	<ul>
		<li>Briefed on local dangers (Witchwood, goblins, orcs, undead rumors).</li>
		<li>Requested directions: lodging, Soranna, other landmarks.</li>
	</ul>
	<li>Party splits:</li>
	<ul>
		<li>Olek + Bonnie &rarr; Tollhouse (contact Soranna, report ambush).</li>
		<li>Norr + Soshi &rarr; Shrine of Pelor (speak with Brother Denny and druid Avartel).</li>
		<li>Kaedin &rarr; Mansion of wizard Sertieren (seek information on Talos and magic).</li>
	</ul>
</ul>
<p><strong>Olek &amp; Bonnie at Tollhouse</strong></p>
<ul>
	<li>Encounter Soranna:</li>
	<ul>
		<li>Bonnie delivers artifact from Rilon Paln.</li>
		<li>
			Soranna identifies one item: Dead Man&rsquo;s Boots, last held by necromancer Nels Huto, heading into Witchwood.
		</li>
	</ul>
	<li>Discuss town threats with Norro (bald man in rich garb):</li>
	<ul>
		<li>Raids are ongoing; &ldquo;Day of Ruin&rdquo; not recognized.</li>
		<li>Base likely deep in Witchwood.</li>
		<li>Offer: stop raiders for reward.</li>
	</ul>
	<li>Olek mounts steed with Bonnie to reunite with party.</li>
</ul>
<p><strong>Norr &amp; Soshi at Shrine of Pelor</strong></p>
<ul>
	<li>Purchase offerings for the shrine (greens and fruits for 5 silver each).</li>
	<li>Meet Brother Denny and Avartel:</li>
	<ul>
		<li>Avartel provides information on Witchwood and Wrath Keep (fortress, cursed land, skeletons).</li>
		<li>Brother Denny accepts offerings; tries to sell wares.</li>
	</ul>
	<li>Depart shrine for meeting point.</li>
</ul>
<p><strong>Kaedin at Sertieren Mansion</strong></p>
<ul>
	<li>Encounters magical fountain (speaks) and halfling servant.</li>
	<li>Asks wizard Sertieren about Talos, disappearances, and magical phenomena:</li>
	<ul>
		<li>Mentions Chronomancers of Kryn (rumors of deaths by dragons).</li>
		<li>Notes magical nexus in Witchwood; area dangerous.</li>
		<li>Comments on city defenses and local threats.</li>
	</ul>
	<li>Kaedin departs mansion; heads to meeting point.</li>
</ul>
<h3>III. Quest &amp; Objective Tracker</h3>
<div class="quest-container">
	<div class="quest">
		<div class="quest-name">Investigate Wyrmlord Koth and "Day of Ruin"</div>
		<div class="quest-description">
			<span>
				<strong>Active</strong>
				- Interrogated hobgoblin survivor; enemy gathering tribes for major assault; location unknown
			</span>
			<span>Last update: session 1</span>
		</div>
	</div>
	<div class="quest">
		<div class="quest-name">Retrieve Dead Man's Boots from Nels Huto</div>
		<div class="quest-description">
			<span>
				<strong>Active</strong>
				- Boots last held by necromancer heading into Witchwood; artifact delivered to Soranna
			</span>
			<span>Last update: session 1</span>
		</div>
	</div>
	<div class="quest">
		<div class="quest-name">Stop the Raiders</div>
		<div class="quest-description">
			<span>
				<strong>Active</strong>
				- Norro offered reward; raids ongoing; base likely in Witchwood
			</span>
			<span>Last update: session 1</span>
		</div>
	</div>
	<div class="quest">
		<div class="quest-name">Investigate Witchwood and Wrath Keep</div>
		<div class="quest-description">
			<span>
				<strong>Active</strong>
				- Avartel provided information about cursed fortress with undead
			</span>
			<span>Last update: session 1</span>
		</div>
	</div>
</div>
<h3>IV. Key People &amp; Factions</h3>
<p><strong>New NPCs Introduced:</strong></p>
<ul>
	<li>
		<strong>Sergeant Hersk</strong>
		- Town guard commander who briefed party on local dangers
	</li>
	<li>
		<strong>Soranna</strong>
		- Tollhouse contact who identified the Dead Man's Boots quest item
	</li>
	<li>
		<strong>Norro</strong>
		- Bald elder/administrator who offered bounty for stopping raiders
	</li>
	<li>
		<strong>Brother Denny</strong>
		- Priest at Shrine of Pelor
	</li>
	<li>
		<strong>Avartel</strong>
		- Druid with knowledge of Witchwood and Wrath Keep
	</li>
	<li>
		<strong>Sertieren</strong>
		- Town wizard with information on magical phenomena and Chronomancers
	</li>
</ul>
<p><strong>Factions Engaged or Updated:</strong></p>
<ul>
	<li>
		<strong>Wyrmlord Koth's Forces</strong>
		- Gathering hobgoblin tribes for "Day of Ruin"
	</li>
	<li>
		<strong>Drellin's Ferry Town Guard</strong>
		- Allied; briefed party on local threats
	</li>
	<li>
		<strong>Arcane Brotherhood</strong>
		- Mentioned as source of training for Norr and Soshi
	</li>
</ul>
<h3>V. Important Discoveries &amp; Loot</h3>
<p><strong>Plot Discoveries:</strong></p>
<ul>
	<li>Wyrmlord Koth is gathering tribes for a "Day of Ruin" attack</li>
	<li>Witchwood contains dangerous magical nexus and cursed fortress (Wrath Keep)</li>
	<li>Chronomancers of Kryn rumored killed by dragons</li>
	<li>Necromancer Nels Huto heading into Witchwood with Dead Man's Boots</li>
</ul>
<p><strong>Key Items/Loot Acquired:</strong></p>
<ul>
	<li>Dead Man's Boots artifact information (not yet retrieved)</li>
	<li>Market offerings purchased (greens and fruits)</li>
	<li>Spoiled wine confiscated from fleeing farmers</li>
</ul>
