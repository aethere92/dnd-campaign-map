class CampaignManager {
	...
	loadCampaign(campaignId, mapKey = null, sessionId = null, characterName = null, viewType = null) {
		...
		if (targetView === 'map' && hasMap) {
			...
			// Clear all story-related item parameters
	...
	#loadStoryCampaign(campaign, sessionId = null, characterName = null, viewType = null) {
		...
		Object.entries(itemParams).forEach(([view, param]) => {
			...
			// If it matches, leave it in the URL for StoryBase to read
		});
		...
		if (initialView === 'timeline') {
			...
			params.delete('session');
			params.delete('character');
		} else if (initialView === 'locations') {
			params.set('view', 'locations');
			params.delete('session');
			params.delete('character');
		} else if (initialView === 'npcs') {
			params.set('view', 'npcs');
			params.delete('session');
			params.delete('character');
