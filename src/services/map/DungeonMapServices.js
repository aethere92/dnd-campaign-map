...
class CustomMap {
    ...
    _calculateBounds(mapConfig) {
        return new L.LatLngBounds(this.map.unproject([0, mapConfig.imageHeight], mapConfig.metadata.sizes.maxZoom), this.map.unproject([mapConfig.imageWidth, 0], mapConfig.metadata.sizes.maxZoom));
    }
    ...
    _setBoundsAndFit() {
        ...
        const extendedBounds = L.latLngBounds([originalSouth - latDiff * padding, originalWest - lngDiff * padding], [originalNorth + latDiff * padding, originalEast + lngDiff * padding]);
    ...
    _createExportButton() {
        ...
        exportButton.onAdd = () => {
            ...
            button.style.cssText = 'background-color: white; color: ; padding: 5px 10px; cursor: pointer; display: block; margin-bottom: 5px; border: 2px solid rgba(0, 0, 0, 0.2); background-clip: padding-box; border-radius: 0.25rem; font-family: system-ui;';

            const zoomSelect = L.DomUtil.create('select', 'leaflet-control-custom', container);
            zoomSelect.style.cssText = 'display: block; width: 100%; padding: 5px; border: 2px solid rgba(0, 0, 0, 0.2); background-clip: padding-box; border-radius: 0.25rem; color: black; font-family: system-ui';
