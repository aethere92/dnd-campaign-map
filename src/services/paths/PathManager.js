    ...
    _initializeGeomanEventListeners() {
        ...
        this.map.on('pm:remove', (e) => {
            if (e.shape === 'Line' || e.shape === 'Polygon') {
                ...
                this.paths = this.paths.filter((path) => !path.points.every((point) => removedLayer.getLatLngs().some((latlng) => latlng.lat === point.coordinates[0] && latlng.lng === point.coordinates[1])));
            }
        });
        ...
        this.map.on('pm:edit', (e) => {
            if (e.shape === 'Line' || e.shape === 'Polygon') {
                ...
                const pathIndex = this.paths.findIndex((path) => path.points.every((point) => editedLayer.getLatLngs().some((latlng) => latlng.lat === point.coordinates[0] && latlng.lng === point.coordinates[1])));
                if (pathIndex !== -1) {
        ...
        areas.forEach((area) => {
            ...
            const textMarker = L.marker(center, {
                icon: L.divIcon({
                    ...
                    html: `<span class="polygon-text" style="transform: rotate(${area?.textRotation ?? '0deg'}); font-size: ${area?.textSize ?? '10'}pt; font-family: var(--font-${area?.textFontType ?? 'description'});">${area.name}</span>`,
                }),
            }).addTo(this.map);
